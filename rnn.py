...
class RNN(nn.Module):
    ...
    def forward(self, inputs):
        _, outputs = self.rnn(inputs)  # Compute outputs using RNN
        summed_outputs = outputs.sum(dim=0)  # Sum outputs across time steps
        predicted_vector = self.W(summed_outputs)  # Apply linear transformation
        predicted_vector = self.softmax(predicted_vector)  # Apply softmax activation
...
if __name__ == "__main__":
    ...
    for input_words, gold_label in tqdm(test_data):
        ...
        # Transform input into required shape
        vectors = torch.tensor(vectors).view(len(vectors), 1, -1)
